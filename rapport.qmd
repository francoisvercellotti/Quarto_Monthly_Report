...
print(f"- 🔢 Nombre d'enregistrements : **{len(df_taxi):,}**")
...
if 'df' in globals() and not df.empty and "hour" in df.columns:
    ...
    fig = px.bar(hourly_counts, x="hour", y="count",
                 ...
                 title="Nombre de trajets par heure de la journée")
...
```{python}
#| fig-cap: "Évolution de la distance moyenne par heure"
...
```{python}
#| fig-cap: "Distribution des durées de trajet"
...
```{python}
#| output: asis
# Synthèse "à la volée" en Markdown
...
print(f"- 📆 Une analyse plus détaillée de la répartition journalière est disponible dans les graphiques ci-dessus.")
```

# 🚀 Recommandations pour les Prochains Mois

* Intégrer le montant des courses et des pourboires pour enrichir les analyses économiques.
* Identifier les zones géographiques les plus actives via des cartes interactives.
* Étudier plus en détail l'impact des facteurs externes comme les événements ou la saisonnalité sur le nombre de courses.
* Croiser les données avec des événements locaux (concerts, matchs, conventions) pour expliquer certains pics.